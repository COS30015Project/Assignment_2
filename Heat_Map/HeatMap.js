function init() {
    ...
    // Define the datasets for each year
    const datasets = ['2021.csv', '2020.csv', '2019.csv'];
    ...
    loadDataset(datasets[currentDatasetIndex]);

    function loadDataset(dataset) {
        Promise.all([
            d3.json("usa.json"), // Load GeoJSON data
            d3.csv(dataset) // Load CSV data
        ]).then(function (data) {
            const json = data[0];
...
// Add a slider for changing the year
